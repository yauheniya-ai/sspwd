...
class TestHashKey:
    def test_deterministic(self) -> None:
        assert ic._hash_key("iconify", "mdi:home") == ic._hash_key(
            "iconify", "mdi:home"
        )
...
class TestCacheIconify:
    ...
    def test_successful_download(self, tmp_path: Path) -> None:
        with patch(
            "sspwd.storage.icon_cache._fetch", return_value=(SVG_BYTES, "image/svg+xml")
        ):
            result = ic.cache_iconify("mdi:home", tmp_path)
    ...
    def test_iconify_404_body_returns_none(self, tmp_path: Path) -> None:
        ...
        with patch(
            "sspwd.storage.icon_cache._fetch", return_value=(b"404", "text/plain")
        ):
    ...
    def test_empty_body_returns_none(self, tmp_path: Path) -> None:
        with patch(
            "sspwd.storage.icon_cache._fetch", return_value=(b"", "image/svg+xml")
        ):
            result = ic.cache_iconify("mdi:home", tmp_path)
        assert result is None
    ...
    def test_non_xml_body_returns_none(self, tmp_path: Path) -> None:
        with patch(
            "sspwd.storage.icon_cache._fetch",
            return_value=(b"not svg at all", "image/svg+xml"),
        ):
            result = ic.cache_iconify("mdi:home", tmp_path)
        assert result is None
...
class TestCacheUrl:
    ...
    def test_svg_detected_from_content_type_substring(self, tmp_path: Path) -> None:
        ...
        with patch(
            "sspwd.storage.icon_cache._fetch",
            return_value=(SVG_BYTES, "image/svg+xml; charset=utf-8"),
        ):
            result = ic.cache_url("https://example.com/icon", tmp_path)
    ...
    def test_png_without_pillow_saved_raw(self, tmp_path: Path) -> None:
        ...
        with patch(
            "sspwd.storage.icon_cache._fetch", return_value=(PNG_1PX, "image/png")
        ):
    ...
    def test_png_with_pillow_saved_resized(self, tmp_path: Path) -> None:
        ...
        with patch(
            ...
            with patch(
                "sspwd.storage.icon_cache._resize_raster", return_value=fake_resized
            ):
                result = ic.cache_url("https://example.com/logo.png", tmp_path)
    ...
    def test_unknown_ext_falls_back_to_png(self, tmp_path: Path) -> None:
        ...
        with patch(
            "sspwd.storage.icon_cache._fetch",
            return_value=(PNG_1PX, "application/octet-stream"),
        ):
            with patch("sspwd.storage.icon_cache._resize_raster", return_value=None):
    ...
    def test_fetch_failure_returns_none(self, tmp_path: Path) -> None:
        with patch(
            "sspwd.storage.icon_cache._fetch", side_effect=OSError("unreachable")
        ):
            result = ic.cache_url("https://example.com/logo.png", tmp_path)
        assert result is None
    ...
    def test_webp_url_ext_used_when_no_ct(self, tmp_path: Path) -> None:
        ...
        with patch(
            "sspwd.storage.icon_cache._fetch",
            return_value=(b"\x00webp", "application/octet-stream"),
        ):
            with patch("sspwd.storage.icon_cache._resize_raster", return_value=None):
...
class TestCacheIcon:
    def test_iconify_routed(self, tmp_path: Path) -> None:
        with patch(
            "sspwd.storage.icon_cache.cache_iconify", return_value="iconify_abc.svg"
        ) as mock:
    ...
    def test_url_routed(self, tmp_path: Path) -> None:
        with patch(
            "sspwd.storage.icon_cache.cache_url", return_value="url_abc.png"
        ) as mock:
