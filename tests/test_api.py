...
def client(tmp_path: Path) -> TestClient:
    ...
    # inject directly into the session dict — no password prompt needed in tests
...
def test_create_full(client: TestClient) -> None:
    payload = {"title": "AWS", "username": "admin", "password": "hunter2",
               "url": "https://aws.amazon.com", "notes": "root account"}
...
def test_create_missing_required_fields(client: TestClient) -> None:
    r = client.post(f"/api/v1/entries?project={PROJECT}", json={"title": "only title"})
...
def test_require_raises_when_project_locked() -> None:
    """_require() must raise 401 for a project not in _sessions."""
...
def test_unlock_wrong_password(tmp_path: Path) -> None:
    """Unlocking an existing vault with the wrong password returns 401."""
    vault_path = tmp_path / "myproject"
    SQLiteStorage(master_password="correct", vault_dir=vault_path)
    server = UIServer(open_browser=False)
    client = TestClient(server.app)
    from unittest.mock import patch
    # Must patch in BOTH modules that call project_dir
...
def test_create_project_and_list(tmp_path: Path) -> None:
    """Create a project via the API, then verify it appears in /projects."""
    server = UIServer(open_browser=False)
    client = TestClient(server.app)
    import sspwd.ui.api as api
    from unittest.mock import patch
...
def test_create_project_already_exists(tmp_path: Path) -> None:
    """Creating a project whose vault.db already exists → 409."""
...
def test_list_unlocked(client: TestClient) -> None:
    """GET /projects/unlocked returns the currently unlocked project names."""
...
def test_lock_project(client: TestClient) -> None:
    """DELETE /projects/{name}/lock removes it from the session."""
...
def test_unlock_success(tmp_path: Path) -> None:
    """Correct password unlocks the project and returns 200."""
...
def test_list_projects_no_sspwd_dir(tmp_path: Path, monkeypatch) -> None:
    """GET /projects returns [] when ~/.sspwd does not exist."""
    import pathlib
...
def test_icon_upload_and_serve(client: TestClient) -> None:
    """Upload a 1×1 PNG, then serve it back."""
    png_bytes = (
